...
try
    load('Tim_ADMCIdata.mat')
    load('Tim_controldata.mat')
    load('Tim_PSPdata.mat')
catch
    for subjnum = 1:47
        if subjnum <10 % Deal with preceding zeros
            Thisfilepath = [pwd '\associationMatrices\00' num2str(subjnum)];
            Metrics = ls(Thisfilepath); % List the contents of the data directory
        else
            Thisfilepath = [pwd '\associationMatrices\0' num2str(subjnum)];
            Metrics = ls(Thisfilepath);
        end
        
        for filenumber = 1:size(Metrics,1)
            filename_deblanked = deblank(Metrics(filenumber,:)); %Get the filename without trailing blanks
            try
                if strcmp(filename_deblanked(1:6),'brain_')
                    if strcmp(filename_deblanked(7:12),'local_')
                        if strcmp(filename_deblanked(14),'_')
                            network_threshold=str2num(filename_deblanked(13));
                            datatype = filename_deblanked(15:end-4);
                            if ~any(strcmp(all_datatypes,datatype))
                                all_datatypes{end+1} = datatype;
                            end
                        else
                            network_threshold=str2num(filename_deblanked(13:14));
                            datatype = filename_deblanked(16:end-4);
                            if ~any(strcmp(all_datatypes,datatype))
                                all_datatypes{end+1} = datatype;
                            end
                        end
                        
                        %New way of reading files - much faster - requires
                        %swallow_csv mex helper from internet
                        filename = [Thisfilepath '\brain_local_' num2str(network_threshold) '_' datatype '.txt'];
                        [numbers, ~] = swallow_csv(filename,'"' , ' ');
                        if any(~isnan(numbers(1,:)))
                            if subjnum <= size(ADMCIdata.AV_binding,2)
                                eval(['ADMCIdata.' datatype '(network_threshold,subjnum,:) = numbers(2,~isnan(numbers(1,:)))'';'])
                                
                            elseif subjnum > size(ADMCIdata.AV_binding,2) && subjnum <= size(ADMCIdata.AV_binding,2)+size(controldata.AV_binding,2)
                                eval(['controldata.' datatype '(network_threshold,subjnum,:) = numbers(2,~isnan(numbers(1,:)))'';'])
                            else
                                eval(['PSPdata.' datatype '(network_threshold,subjnum,:) = numbers(2,~isnan(numbers(1,:)))'';'])
                            end
                        else
                            if subjnum <= size(ADMCIdata.AV_binding,2)
                                eval(['ADMCIdata.' datatype '(network_threshold,subjnum,:) = numbers(2,2:end)'';'])
                                
                            elseif subjnum > size(ADMCIdata.AV_binding,2) && subjnum <= size(ADMCIdata.AV_binding,2)+size(controldata.AV_binding,2)
                                eval(['controldata.' datatype '(network_threshold,subjnum,:) = numbers(2,2:end)'';'])
                            else
                                eval(['PSPdata.' datatype '(network_threshold,subjnum,:) = numbers(2,2:end)'';'])
                            end
                        end
                        
                        % Old way of reading in files - works but extremely
                        % slow - use if you don't have swallow_csv
%                         try
%                             thisdata = load([Thisfilepath '\brain_local_' num2str(network_threshold) '_' datatype '.txt']);
%                             if subjnum <= size(ADMCIdata.AV_binding,2)
%                                 eval(['ADMCIdata.' datatype '(network_threshold,subjnum,:) = thisdata(2,:)'';'])
%                                 
%                             elseif subjnum > size(ADMCIdata.AV_binding,2) && subjnum <= size(ADMCIdata.AV_binding,2)+size(controldata.AV_binding,2)
%                                 eval(['controldata.' datatype '(network_threshold,subjnum,:) = thisdata(2,:)'';'])
%                             else
%                                 eval(['PSPdata.' datatype '(network_threshold,subjnum,:) = thisdata(2,:)'';'])
%                             end
%                             
%                         catch
%                             thisdata = readtable([Thisfilepath '\brain_local_' num2str(network_threshold) '_' datatype '.txt'],'FileType','text','Delimiter',' ','HeaderLines',0,'ReadRowNames',1,'ReadVariableNames',1);
%                             
%                             if subjnum <= size(ADMCIdata.AV_binding,2)
%                                 eval(['ADMCIdata.' datatype '(network_threshold,subjnum,:) = thisdata{1,:}'';'])
%                                 
%                             elseif subjnum > size(ADMCIdata.AV_binding,2) && subjnum <= size(ADMCIdata.AV_binding,2)+size(controldata.AV_binding,2)
%                                 eval(['controldata.' datatype '(network_threshold,subjnum,:) = thisdata{1,:}'';'])
%                             else
%                                 eval(['PSPdata.' datatype '(network_threshold,subjnum,:) = thisdata{1,:}'';'])
%                             end
%                         end
...
all_fields = fields(controldata);
for i = 1:length(fields(controldata))
    try
        eval(['controldata.' all_fields{i} ' = controldata.' all_fields{i} '(:,18:30,:);'])
    end
end
all_fields = fields(PSPdata);
for i = 1:length(fields(PSPdata))
    try
        eval(['PSPdata.' all_fields{i} ' = PSPdata.' all_fields{i} '(:,31:47,:);'])
    end
end
...
% 
% if ~exist([pwd '\Rittman_figures\'] ,'dir')
%     mkdir([pwd '\Rittman_figures\']);
% end
% 
% for density_threshold = 1:10;
%     for y_variable = controlfields';
%       
%         if strcmp(y_variable,'AV_binding')
%             continue
%         elseif ndims(eval(['controldata.' y_variable{1}])) == 2;
%             continue
%         else
%             
%             h = figure;
...
%                 if groupstodraw(i) == 1
%                     y_data = eval(['squeeze(controldata.' y_variable{1} '(density_threshold,:,:))']);
%                     h1 = plot(1:length(y_data),nanmean(y_data),'k.');
%                 elseif groupstodraw(i) == 2
%                     y_data = eval(['squeeze(ADMCIdata.' y_variable{1} '(density_threshold,:,:))']);
%                     h2 = plot(1:length(y_data),nanmean(y_data),'r.');
%                 elseif groupstodraw(i) == 3
%                     y_data = eval(['squeeze(PSPdata.' y_variable{1} '(density_threshold,:,:))']);
...
%         end
%         
%         saveas(h,['Rittman_figures\' titletext '.jpg']);
%         close(h)
%         
%     end
% end
...
for density_threshold = 1:10;
    for y_variable = controlfields';
        ...
        elseif ndims(eval(['controldata.' y_variable{1}])) == 2;
            continue
        elseif size(eval(['controldata.' y_variable{1}]),3) ~= 598
            continue            
        else
            ...
            this_control_y_data = eval(['nanmean(squeeze(controldata.' y_variable{1} '(density_threshold,:,:)) ,1)''']);
            this_ADMCI_y_data = eval(['nanmean(squeeze(ADMCIdata.' y_variable{1} '(density_threshold,:,:)) ,1)''']);
            this_PSP_y_data = eval(['nanmean(squeeze(PSPdata.' y_variable{1} '(density_threshold,:,:)) ,1)''']);
            ...
            for i = 1:length(groupstodraw)
                ...
                saveas(h,['Rittman_figures\Multigradient_' titletext '.jpg']);
                close(h)
                ...
                saveas(h,['Rittman_figures\Significant_multigradient_' titletext '.jpg']);
                close(h)
                ...
                if groupstodraw(i) == 2 || groupstodraw(i) == 3
                    ...
                    control_y_data = eval(['squeeze(controldata.' y_variable{1} '(density_threshold,:,:))''']);
                    if groupstodraw(i) == 2
                        x_data = eval(['ADMCIdata.' x_variable]);
                        y_data = eval(['squeeze(ADMCIdata.' y_variable{1} '(density_threshold,:,:))''']);
            ...
            if ~exist('group','var') || strcmp(group,'all')==1
                ...
                saveas(h,['Rittman_figures\Allgradients_' y_variable{1} '_against_' x_variable ' at a density threshold of ' num2str(density_threshold) '.jpg']);
                close(h)
                ...
                saveas(h,['Rittman_figures\Allgradients_pordered_' y_variable{1} '_against_' x_variable ' at a density threshold of ' num2str(density_threshold) '.jpg']);
                close(h)
                ...
                saveas(h,['Rittman_figures\Allgradients_normalised_' y_variable{1} '_against_' x_variable ' at a density threshold of ' num2str(density_threshold) '.jpg']);
                close(h)
            end
        end
    end
...
%Next step - region-wise and brain-wise averages.
...
%% Next step: Average across brain regions and across the whole brain and look at gradients.

%Get regionwise average AV and whole brain for each individual
lobes = unique(atlasdata(:,2),'stable');
for i = 1:length(lobes);
    ADMCI_regionalAV(i,:) = nanmean(ADMCIdata.AV_binding(~cellfun(@isempty,regexp(atlasdata(:,2),lobes{i})),:));
    control_regionalAV(i,:) = nanmean(controldata.AV_binding(~cellfun(@isempty,regexp(atlasdata(:,2),lobes{i})),:));
    PSP_regionalAV(i,:) = nanmean(PSPdata.AV_binding(~cellfun(@isempty,regexp(atlasdata(:,2),lobes{i})),:));
end
ADMCI_totalAV = nanmean(ADMCI_regionalAV);
control_totalAV = nanmean(control_regionalAV);
PSP_totalAV = nanmean(PSP_regionalAV);

%first average regionwise
% thisdata(i,:) = mean(ADMCIdata.AV_binding(~cellfun(@isempty,regexp(atlasdata(:,2),lobes{i})),:));
...
for density_threshold = 1:10;
    for y_variable = controlfields';
        ...
        elseif size(eval(['controldata.' y_variable{1}]),3) ~= 598
            ...
            this_control_x_data = nanmean(control_regionalAV,1)';
            this_ADMCI_x_data = nanmean(ADMCI_regionalAV,1)';
            this_PSP_x_data = nanmean(PSP_regionalAV,1)';
            this_x_data = [this_control_x_data;this_ADMCI_x_data;this_PSP_x_data];
            for i = 1:length(lobes);
                this_control_regionwise_data(i,:) = eval(['nanmean(squeeze(controldata.' y_variable{1} '(density_threshold,:,~cellfun(@isempty,regexp(atlasdata(:,2),lobes{i})))),2)']);
                this_ADMCI_regionwise_data(i,:) = eval(['nanmean(squeeze(ADMCIdata.' y_variable{1} '(density_threshold,:,~cellfun(@isempty,regexp(atlasdata(:,2),lobes{i})))),2)']);
                this_PSP_regionwise_data(i,:) = eval(['nanmean(squeeze(PSPdata.' y_variable{1} '(density_threshold,:,~cellfun(@isempty,regexp(atlasdata(:,2),lobes{i})))),2)']);
            end
            this_control_y_data = nanmean(this_control_regionwise_data,1)';
            this_ADMCI_y_data = nanmean(this_ADMCI_regionwise_data,1)';
            this_PSP_y_data = nanmean(this_PSP_regionwise_data,1)';
            this_y_data = [this_control_y_data;this_ADMCI_y_data;this_PSP_y_data];
            titletext = ['All subjects region averaged' y_variable{1} ' against ' x_variable ' at a density threshold of ' num2str(density_threshold)];
            ...
            ylabel(y_variable{1}, 'Interpreter', 'none')
            title(titletext, 'Interpreter', 'none')
            saveas(h,['Rittman_figures\region_averaged' titletext '.jpg']);
            close(h)
            
            
            for i = 1:length(groupstodraw)
                if groupstodraw(i) == 1
                    x_data = control_regionalAV;
                    y_data = this_control_regionwise_data;
                    titletext = ['Region_wise Controls ' y_variable{1} ' against ' x_variable ' at a density threshold of ' num2str(density_threshold)];
                elseif groupstodraw(i) == 2
                    x_data = ADMCI_regionalAV;
                    y_data = this_ADMCI_regionwise_data;
                    titletext = ['Region_wise ADMCI ' y_variable{1} ' against ' x_variable ' at a density threshold of ' num2str(density_threshold)];
                elseif groupstodraw(i) == 3
                    x_data = PSP_regionalAV;
                    y_data = this_PSP_regionwise_data;
                    titletext = ['Region_wise PSP ' y_variable{1} ' against ' x_variable ' at a density threshold of ' num2str(density_threshold)];
                end
                
                %Simple scatter plot
                h = figure;
                gscatter(nanmean(x_data'),nanmean(y_data'),lobes,hsv(numel(unique(atlasdata(:,2)))))
                regres_coeffs = [nanmean(x_data')',ones(size(nanmean(x_data')'))] \ nanmean(y_data')';
                hold on
                xs = min(nanmean(x_data')):(range(nanmean(x_data'))/100):max(nanmean(x_data'));
                plot(xs, (regres_coeffs(1)*xs + regres_coeffs(2)));
                ...
                for j = 1:size(y_data,1)
                    regres_coeffs = [x_data(j,:)',ones(size(x_data(j,:)'))] \ y_data(j,:)';
                    xs = min(x_data(j,:)):(range((x_data(j,:)))/100):max(x_data(j,:));
                    plot(xs, (regres_coeffs(1)*xs + regres_coeffs(2)),'Color',cols(j,:));
                    all_gradients(j) = regres_coeffs(1);
                    [R,P] = corrcoef(x_data(j,:),y_data(j,:));
                    R_gradients(j) = R(2,1);
                    P_gradients(j) = P(2,1);
                ...
                xlabel(x_variable, 'Interpreter', 'none')
                ylabel(y_variable{1}, 'Interpreter', 'none')
                title(['Region Averaged Multigradient plot for ' titletext], 'Interpreter', 'none')
                saveas(h,['Rittman_figures\region_averaged_Multigradient_' titletext '.jpg']);
                close(h)
                ...
                ylabel(y_variable{1}, 'Interpreter', 'none')
                title(['Significant multigradient plot for ' titletext], 'Interpreter', 'none')
                saveas(h,['Rittman_figures\region_averaged_Significant_multigradient_' titletext '.jpg']);
                close(h)
...
%                 end
            ...
            if ~exist('group','var') || strcmp(group,'all')==1
                ...
                h=figure;
                hold on
                gscatter(1+xs,controlgradientsnormed',lobes,hsv(numel(unique(atlasdata(:,2)))));
                if controlh == 1
                    %scatter(xs,controlgradientsnormed,'r')
                    if nanmean(controlgradientsnormed)>0
                        plot(1,max([controlgradientsnormed,ADMCIgradientsnormed,PSPgradientsnormed]),'*k')
                ...
                gscatter(2+xs,ADMCIgradientsnormed',lobes,hsv(numel(unique(atlasdata(:,2)))));
                if ADMCIh == 1
                    %scatter(2*xs,ADMCIgradientsnormed,'r')
                    if nanmean(ADMCIgradientsnormed)>0
                        plot(2,max([controlgradientsnormed,ADMCIgradientsnormed,PSPgradientsnormed]),'*k')
                ...
                gscatter(3+xs,PSPgradientsnormed',lobes,hsv(numel(unique(atlasdata(:,2)))));
                if PSPh == 1
                    %scatter(3*xs,PSPgradientsnormed,'r')
                    if nanmean(PSPgradientsnormed)>0
                        plot(3,max([controlgradientsnormed,ADMCIgradientsnormed,PSPgradientsnormed]),'*k')
                ...
                ax.XTickLabel = {'Control';'ADMCI';'PSP'};
                ylabel(['Gradient'])
                title(['Region averaged Allgradients plot for ' y_variable{1} ' normalised against ' x_variable ' at a density threshold of ' num2str(density_threshold)], 'Interpreter', 'none')
                saveas(h,['Rittman_figures\region_averaged_Allgradients_normalised_' y_variable{1} '_against_' x_variable '_at_' num2str(density_threshold) '.jpg']);
                close(h)
          ...
          clear this_control_regionwise_data this_ADMCI_regionwise_data this_PSP_regionwise_data  
        end
    end
end

% Now average across the whole brain
...
for density_threshold = 1:10;
    for y_variable = controlfields';
        ...
        if strcmp(y_variable,'AV_binding')
            ...
            this_control_x_data = control_totalAV';
            this_ADMCI_x_data = ADMCI_totalAV';
            this_PSP_x_data = PSP_totalAV';
            this_x_data = [this_control_x_data;this_ADMCI_x_data;this_PSP_x_data];
            if ndims(eval(['controldata.' y_variable{1}])) == 2;
                this_control_wholebrain_data = eval(['squeeze(controldata.' y_variable{1} '(density_threshold,:,:))''']);
                this_ADMCI_wholebrain_data = eval(['squeeze(ADMCIdata.' y_variable{1} '(density_threshold,:,:))''']);
                this_PSP_wholebrain_data = eval(['squeeze(PSPdata.' y_variable{1} '(density_threshold,:,:))''']);
            else
                this_control_wholebrain_data = eval(['nanmean(squeeze(controldata.' y_variable{1} '(density_threshold,:,:)),2)']);
                this_ADMCI_wholebrain_data = eval(['nanmean(squeeze(ADMCIdata.' y_variable{1} '(density_threshold,:,:)),2)']);
                this_PSP_wholebrain_data = eval(['nanmean(squeeze(PSPdata.' y_variable{1} '(density_threshold,:,:)),2)']);
            end
            
            this_control_y_data = this_control_wholebrain_data;
            this_ADMCI_y_data = this_ADMCI_wholebrain_data;
            this_PSP_y_data = this_PSP_wholebrain_data;
            this_y_data = [this_control_y_data;this_ADMCI_y_data;this_PSP_y_data];
            titletext = ['All subjects wholebrain averaged' y_variable{1} ' against ' x_variable ' at a density threshold of ' num2str(density_threshold)];
            ...
            ylabel(y_variable{1}, 'Interpreter', 'none')
            title(titletext, 'Interpreter', 'none')
            saveas(h,['Rittman_figures\wholebrain_averaged' titletext '.jpg']);
            close(h)
...
%             ylabel(y_variable{1}, 'Interpreter', 'none')
%             title(titletext, 'Interpreter', 'none')
%             saveas(h,['Rittman_figures\Coloured_' titletext '.jpg']);
%             close(h)
%             
% %                     %Multigradient plot
% %                     h = figure
...
% end
% 
% if ~exist([pwd '\' 'Rittman_figures\RSA\'] ,'dir')
%     mkdir([pwd '\' 'Rittman_figures\RSA\']);
% end
% 
...
%     saveas(h,['Rittman_figures\RSA\Correlation_self_' this_variable{1} '.jpg']);
%     close(h)
%     
...
%     saveas(h,['Rittman_figures\RSA\Dendrogram_' this_variable{1} '.jpg']);
%     close(h)
%     
...
%     saveas(h,['Rittman_figures\RSA\Multi_Dimensional_Scaling_' this_variable{1} '.jpg']);
%     close(h)
...
%             saveas(h,['Rittman_figures\RSA\Correlation_matrix_' first_variable{1} '_against_' second_variable{1} '.jpg']);
%             close(h)
%             
%         end
%         
